...
func GetCmdGetMsg(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := args[1]
			params := types.NewQueryMsgParams(addr, []byte(msg))
			bz, err := cliCtx.Codec.MarshalJSON(params)
			if err != nil {
				return err
			}

			route := fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryGetMsg)
			res, _, err := cliCtx.QueryWithData(route, bz)
			if err != nil {
				return err
			}
...
func GetCmdGetStore(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			route := fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryGetStore)
			res, _, err := cliCtx.QueryWithData(route, bz)
			if err != nil {
				return err
			}
